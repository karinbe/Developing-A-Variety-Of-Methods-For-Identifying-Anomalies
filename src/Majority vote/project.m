...
%     if MLarray(i) + entropyArray(i) >= 2 % will be anomaly for sure
%         disp("Voting: " + i + " is anomaly.")
%     end
% end

for i = 1:length(MLarray)-1
    if MLarray(i) + entropyArray(i) == 2 % will be sick for sure
        disp("Voting: " + i + " is anomaly.")
        if data(i,9) == 0 % TODO cols..
            counterSS = counterSS + 1;
        else
            counterHS = counterHS + 1;
        end
    elseif MLarray(i) + entropyArray(i) == 0 % will be healthy for sure
        if data(i,9) == 0 % TODO cols..
